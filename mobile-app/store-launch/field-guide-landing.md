...
The landing page uses a dependency-free AJAX form with accessible labels and status messages, browser validation, a honeypot field, topic/platform context, a warning not to send sensitive information, and a direct-email fallback. The production Content Security Policy allows browser connections to the dedicated `https://formspree.io` endpoint while keeping other external connection targets blocked. The Formspree endpoint is a public form identifier, not a secret. Formspree account credentials and email-verification links must never be committed.
