...
The hosted Build `6` update was deployed from commit `f8a8424`. Its signed IPA was uploaded successfully at 2026-09-15 11:23 MST, completed App Store Connect processing, and is in `Internal QA` as `Ready to Submit`. Do not reply or resubmit until Build `6` is selected for version `1.0`, its live privacy policy matches the app, and the physical TestFlight smoke test and new review recording pass. Then use `guideline-ai-permission-response-build-6.md`.
