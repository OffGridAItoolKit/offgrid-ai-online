...
Build `1.0 (2)` adds native Apple Speech recognition for Voice Input and the required Speech Recognition permission description. It passed the physical iPhone retest and was archived, distribution-signed by `INSPIRED MARKETING & DESIGN, LLC`, validated, uploaded, and processed successfully by App Store Connect on 2026-09-03. It is listed in TestFlight as `Ready to Submit`. Use this exact TestFlight build for the final physical-device App Review recording and smoke test before resubmission. The complete seven-part response and recording checklist are in `guideline-2.1-response-build-2.md`.
