...
const checks = [
  ...
  ['FieldGuide retries only through a Google Vertex multimodal fallback', server.includes("const FIELDGUIDE_VERTEX_FALLBACK_MODEL = 'google/gemini-2.5-flash'") && server.includes('getProviderRequestModels(modelId, enforceZdr)')],
  ['provider retry is limited to transient statuses', server.includes('[408, 409, 429, 500, 502, 503, 504].includes(status)')],
